...
data_dict   = darntids.classify.sort_by_spectrum(data_dict,sort_key)
data_dict   = darntids.classify.classify_mstid_events(data_dict)

darntids.classify.rcss(data_dict,classification_path=classification_path)

plot_nr     = 1
filename    = '{:03d}_spectral_plot.png'.format(plot_nr)
darntids.classify.spectral_plot(data_dict,output_dir=output_dir,plot_all_spect_mean=True,filename=filename)

plot_nr     += 1
filename    = '{:03d}_spectral_plot_mean_subtracted.png'.format(plot_nr)
darntids.classify.spectral_plot(data_dict,output_dir=output_dir,filename=filename,subtract_mean=True)

plot_nr     += 1
filename    = '{:03d}_spectral_plot_mean_subtracted_ranked.png'.format(plot_nr)
darntids.classify.spectral_plot(data_dict,output_dir=output_dir,subtract_mean=True,color_key='spectral_sort',filename=filename)
